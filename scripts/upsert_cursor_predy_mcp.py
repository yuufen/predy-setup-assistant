...
def load_config(path: Path) -> Dict[str, Any]:
    ...
    if not isinstance(data, dict):
        raise ValueError("Cursor MCP config must be a JSON object.")
    return data


def build_server(command: str, args: List[str], existing: Optional[Dict[str, Any]]) -> Dict[str, Any]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Insert or replace a Cursor Predy MCP server entry.")
    parser.add_argument("--config", default=DEFAULT_CONFIG, help=f"Path to mcp.json (default: {DEFAULT_CONFIG})")
    ...
    mcp_servers[args.name] = build_server(command_path, args.args, existing)
