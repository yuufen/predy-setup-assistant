...
- install the right client assets for Codex, Claude, Cursor, CodeWiz, and Copilot
...
14. If the current host already reveals the client, do not ask the user to identify it again. Treat `.codewiz/skills/...` as CodeWiz, `.cursor/...` as Cursor, `.github/skills/...` as Copilot, `~/.claude/...` as Claude, and `~/.codex/...` as Codex.
...
19. For Claude or Copilot MCP wiring, generate the wrapper first and then render a manual prompt with `scripts/render_manual_mcp_prompt.py`.
